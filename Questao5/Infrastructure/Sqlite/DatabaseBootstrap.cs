    ...
    internal class DatabaseBootstrap(DatabaseConfig databaseConfig) : IDatabaseBootstrap
    {
        public void Setup()
        {
            ...
            var table = connection.Query<string>(
                ...
                FROM sqlite_master 
                WHERE type='table' 
                AND (name = 'contacorrente' or name = 'movimento' or name = 'idempotencia');
                """);
            
            var tableName = table.FirstOrDefault();
            
            if (!string.IsNullOrEmpty(tableName) && tableName is "contacorrente" or "movimento" or "idempotencia")
                return;
